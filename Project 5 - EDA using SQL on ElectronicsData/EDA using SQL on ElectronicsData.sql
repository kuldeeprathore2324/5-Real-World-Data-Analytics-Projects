...
LOAD DATA INFILE "F:/data analysis projects 6 by campusx/EDA using SQL/ElectronicsData.csv"
...
select price ,replace(substring_index(replace(price, "-", ""),"through", 1),",","")+
replace(substring_index(replace(price, "-", ""),"through", -1), ",","")as total
...
WHERE `Sub Category`  = 'Home Security Systems & Cameras' and Brandname = "Lorex"  -- removing the row
...
select @q1 as "Quater1",@q2 as "Median" , @q3 as"Quater3"  
...
SELECT `sub category`, COUNT(`sub category`)
 AS sub_category_count FROM electronicsdata GROUP BY `sub category`;


 -------------------------       -- BIVARIATE ANALYSIS  (NUMERICAL - NUMERICAL ANALYSIS)          -----------------------
                      
-- 1 SCATTER PLOt
SELECT  REVIEWS_COUNT,AVERAGE_RATING FROM ELECTRONICSDATA    -- increase in count of reviews leads to increase in average reatings.
...
SELECT 
    ...
    SUM(POWER(PRICE - (SELECT AVG(PRICE) FROM ELECTRONICSDATA), 2))
     AS slope
